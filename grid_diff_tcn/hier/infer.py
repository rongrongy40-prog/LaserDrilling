...
from grid_diff_tcn.hier.decision import (  # noqa: E402
    apply_safety_lock,
    s3wd_decide,
    topkmedian_decide,
...
def _build_model(
    ...
    model = HierarchicalGridDiffProbTransformer(
        in_channels_frame=int(cfg.get("in_channels_frame", 64)),
        out_channels=2,
        ...
        add_kl=True,
        return_kl=False,
        extra_dim=int(getattr(args, "extra_dim", 41)),
    ).to(device)
            ...
            for i in range(bsz):
                ...
                if decision_method == "topkmedian" and use_uncertainty_gate and probs_var_bt is not None:
                    ...
                    pred_pen, pred_idx = _decide(
                        p, method=decision_method, topk_k=topk_k, topk_min_thresh=topk_min_thresh
                    )
...
def main():
    ...
    ap.add_argument("--roi_window_side", type=int, default=None)
    ...
    ap.add_argument("--use_hole_anchor_box", action="store_true")
    ...
    ap.add_argument("--decision_method", type=str, default="topkmedian", choices=["topkmedian", "s3wd"])
