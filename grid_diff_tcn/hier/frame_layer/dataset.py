...
Returns per-sample tensors:
  - frame_data: (T, F, 64)  per-layer per-frame grid features
...
def _pool_regions_8x8(x_btf64: torch.Tensor) -> torch.Tensor:
    """
    x_btf64: (B,T,F,64) float
    return: (B,T,F,5) float
    5 regions = [center2x2, top, bottom, left, right]
    """
    b, t, f, c = x_btf64.shape
    assert c == 64
    g = x_btf64.view(b, t, f, 8, 8)
    center = g[..., 3:5, 3:5].mean(dim=(-1, -2))
    top = g[..., :2, :].mean(dim=(-1, -2))
    bottom = g[..., -2:, :].mean(dim=(-1, -2))
    left = g[..., :, :2].mean(dim=(-1, -2))
    right = g[..., :, -2:].mean(dim=(-1, -2))
...
def _grid_pool_single(gray_roi: np.ndarray, grid: Tuple[int, int] = (8, 8)) -> np.ndarray:
    h, w = gray_roi.shape[:2]
    gr, gc = int(grid[0]), int(grid[1])
    if h < gr or w < gc:
        return np.zeros((gr * gc,), dtype=np.float32)
    ph, pw = h // gr, w // gc
    out = np.zeros((gr * gc,), dtype=np.float32)
    k = 0
    for r in range(gr):
        for c in range(gc):
            patch = gray_roi[r * ph : (r + 1) * ph, c * pw : (c + 1) * pw]
            out[k] = float(np.mean(patch)) if patch.size else 0.0
            k += 1
    return out
...
class HierarchicalFrameLayerDataset(Dataset):
    def __init__(
        ...
        use_hole_anchor_box: bool = False,
        hole_anchor_num_images: int = 10,
        ...
        **_ignored_legacy_kwargs,
        ...
        self.roi_window_side = int(roi_window_side) if roi_window_side is not None else None
        ...
        self.use_hole_anchor_box = bool(use_hole_anchor_box)
        self.hole_anchor_num_images = max(1, int(hole_anchor_num_images))
        self._hole_fixed_box_cache: dict[str, tuple[int, int, int, int] | None] = {}
        self.precomputed_dir = os.path.abspath(precomputed_dir) if precomputed_dir else None

        # 与 precompute 脚本保持一致：按样本名唯一命名（可回退 idx.pt）
        used = set()
        self._precomputed_name_for_idx: List[str] = []
        for i in range(len(self.samples)):
            path = self.samples[i].get("sample_path", "")
            base = os.path.basename(path or "").strip().replace(os.sep, "_").replace("/", "_") or f"sample_{i}"
            name = base
            k = 2
            while name in used:
                name = f"{base}__{k}"
    ...
    def _ensure_hole_fixed_crop_box(self, sample_path: str) -> tuple[int, int, int, int] | None:
        key = os.path.normpath(sample_path)
        if key in self._hole_fixed_box_cache:
            return self._hole_fixed_box_cache[key]
        if not os.path.isdir(sample_path):
            self._hole_fixed_box_cache[key] = None
            return None
        box = compute_hole_anchor_crop_box(
            sample_path,
            self.target_size,
            self.hole_anchor_num_images,
            self.exclude_set,
            self.final_roi_scale,
            self.cc_min_area,
            self.cc_expand_ratio,
            self.use_color_cc_v2_geometry,
            self.roi_window_side,
        )
        self._hole_fixed_box_cache[key] = box
        return box
    ...
    def _extract_frame_feature(
        ...
        roi = color_cc_extract_gray_letterbox(
            ...
            roi_window_side=(None if fixed_crop_box is not None else self.roi_window_side),
            ...
            fixed_crop_box=fixed_crop_box,
        )
        if roi is None or roi.size == 0:
            return None
        return _grid_pool_single(roi, grid=self.grid)
    ...
    def __getitem__(self, index: int) -> dict:
        if self.precomputed_dir:
            ...
            if os.path.isfile(pt_path):
                ...
                if src_f != target_f:
                    ...
                    c = int(frame_data.shape[2]) if frame_data.ndim >= 3 else 64
        ...
        if not layer_list:
            frame_data = torch.zeros(1, self.max_frames_per_layer, 64, dtype=torch.float32)
        ...
        data = np.zeros((t, f, 64), dtype=np.float32)
        mask = np.zeros((t, f), dtype=np.bool_)

        fixed_box = None
        if self.use_hole_anchor_box:
            fixed_box = self._ensure_hole_fixed_crop_box(sample_path)

        for ti, ly in enumerate(layer_list):
            picks = self._select_frames(by_layer.get(ly, []))
            for fi, p in enumerate(picks[:f]):
                feat = self._extract_frame_feature(p, fixed_crop_box=fixed_box)
                if feat is None:
                    continue
                data[ti, fi] = feat
                mask[ti, fi] = True
...
def collate_hierarchical_batch(batch: List[dict]) -> dict:
    ...
    c = int(batch[0]["frame_data"].shape[2]) if batch else 64
    bsz = len(batch)
